...
dane <- dane %>% 
  rename(y = 4) %>% 
  rename_with(.cols = 5:ncol(.), .fn = ~ paste0("x", seq_along(.)))
...
```{r}
model <- lm(y ~ x2 + x3 + x5 + x6 + x7 + x8 + x9, data = dane)
summary(model) 
```
...
```{r}
model <- lm(y ~ x2 + x3 + x5 + x6 + x7 + x8, data = dane)
summary(model)
```
...
```{r}
model <- lm(y ~ x2 + x3 + x5 + x7 + x8, data = dane)
summary(model)
```
...
```{r}
model_koncowy <- lm(y ~ x3 + x5 + x7 + x8, data = dane)
...
```{r}
model_x1 <- lm(y~x7, data = dane)
ggplot(dane, aes(x = Lata, y = x8))+
  geom_point()+
...
```{r}
wykres2 <- ggplot(dane, aes(y = y, x = x9))+
  geom_point()+
  geom_smooth(method = "lm", se = F)+
  theme_minimal()
wykres2
